...
<template>
  <v-card elevation="0" style="width: 100%">
    <v-card-title class="text-center">
      ...
      <span class="text-h5" style="type: #fbc02d">Add machineType</span>
    </v-card-title>
    <v-card-text>
      <v-container>
        <v-row>
      ...
      axios
        ...
        .then((response) => {
          if (response.data.errors) {
            ...
            swal("success", "machine type added successfully", "success");
            this.$router.push({ path: "/utils/machinetype/all" });
          }
        })
        .catch((err) => {
          console.log(err);
